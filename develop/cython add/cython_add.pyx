...
def cython_add_at(arr, k,l,m, I):
    if arr.dtype != I.dtype:
        I = I.astype(arr.dtype)
    if arr.dtype == np.float32:
        return cython_add_at_32(arr, k, l, m, I)
    else:
        return cython_add_at_64(arr, k, l, m, I)

@cython.boundscheck(False) 
@cython.wraparound(False) 
@cython.nonecheck(False)
def cython_add_at_64(np.ndarray[np.float64_t, ndim=3] arr,
...
@cython.wraparound(False) 
@cython.nonecheck(False)
def cython_add_at_32(np.ndarray[np.float32_t, ndim=3] arr,
              np.ndarray[np.int32_t, ndim=1] k_arr,
              np.ndarray[np.int32_t, ndim=1] l_arr,
              np.ndarray[np.int32_t, ndim=1] m_arr,
