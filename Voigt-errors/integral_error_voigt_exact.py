...
awG_kinds = {'linear':'+','min-RMS':'x','optimized':'.'}
awL_kinds = {'linear':'s','min-RMS':'D','optimized':'o'}
...
for tauG in tauG_arr:
    for tauL in tauL_arr:
        awG0,awL0 = W.weights(tauG,tauL,log_pG,log_pL)

        awG_arr = np.arange(awG0 - 0.1,awG0 + 0.1,da)
        awL_arr = np.arange(awL0 - 0.1,awL0 + 0.1,da)
...
##                    log_pG*(1 - 2*tauG)*(S_G1G2*S_L1L1 - S_G1L1*S_G2L1))
        ...
        awG_list = [tauG,0.5*(tauG + (1-pG**-tauG)/(1-pG**-1)),awG]
        awL_list = [tauL,0.5*(tauL + (1-pL**-tauL)/(1-pL**-1)),awL]
