...
def init_w_axis(dx, log_wi, dv):
    log_w_min = np.min(log_wi - np.log(dv))
    log_w_max = np.max(log_wi - np.log(dv)) + 1e-4
...
def calc_matrix(v, log_wG, log_wL, v0i, log_wGi, log_wLi, S0i):
    ...
    li0, li1, aGi = get_indices(log_wGi - np.log(dv), log_wG) #Eqs 3.7 & 3.10
    mi0, mi1, aLi = get_indices(log_wLi - np.log(dv), log_wL) #Eqs 3.7 & 3.10
...
def synthesize_spectrum(v, v0i, log_wGi, log_wLi, S0i, dxG = 0.14, dxL = 0.2, 
    ...
    log_wG = init_w_axis(dxG, log_wGi, dv) #pts
    log_wL = init_w_axis(dxL, log_wLi, dv) #pts
